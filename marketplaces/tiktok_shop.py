...
RE_TRACK_GENERIC = re.compile(r"\b([A-Z]{2}\d{9,}|BR\d{10,})\b")


class TikTokShopDriver:
    name = "tiktok_shop"
    kind = "tiktok_shop"
    ...
    def detect(self, pdf_path: str) -> Optional[DetectedDoc]:
        try:
            ...
            if "ORDER" in up or "PEDIDO" in up:
                score += 0.10
            if RE_TRACK_GENERIC.search(sample):
                score += 0.10
    ...
    def extract(self, docdet: DetectedDoc) -> List[ExtractedEtiqueta]:
        ...
        for i in range(len(doc)):
            ...
            produtos = _parse_produtos_generic(tx)
...
def _parse_produtos_generic(text: str) -> List[Dict[str, Any]]:
    lines = [l.strip() for l in (text or "").splitlines() if l.strip()]
    prods: List[Dict[str, Any]] = []

    for l in lines:
        up = l.upper()
        if "SKU" in up and ("QTY" in up or "QTD" in up):
            sku = ""
            qtd = "1"
            msku = re.search(r"SKU\s*[:#]?\s*([A-Z0-9\-_\.]{2,})", l, re.IGNORECASE)
            if msku:
                sku = msku.group(1)
            mq = re.search(r"(QTY|QTD)\s*[:#]?\s*(\d+)", l, re.IGNORECASE)
            if mq:
                qtd = mq.group(2)
            if sku:
